...
class PrayTimes():
	...
	def computePrayerTimes(self, times):
		...
		quran = self.sunAngleTime(self.eval(params['fajr']), times['quran'], 'ccw')
		quran = quran + 0.17
		return	{
			'imsak': imsak, 'fajr': fajr, 'sunrise': sunrise, 'dhuhr': dhuhr,
			'asr': asr, 'sunset': sunset, 'maghrib': maghrib, 'isha': isha, 'quran': quran
			}

	def computeTimes(self):
		times = {
			'imsak': 5, 'fajr': 5, 'sunrise': 6, 'dhuhr': 12,
			'asr': 13, 'sunset': 18, 'maghrib': 18, 'isha': 18, 'quran': 5
		}
		# main iterations
	...
	def adjustHighLats(self, times):
		...
		times['quran'] = self.addTime(times['quran'], times['sunrise'], self.eval(params['fajr']), nightTime, 'ccw')
		return times
	...
	# adjust a time for higher latitudes
	def addTime(self, time, base, angle, night, direction = None):
		portion = self.nightPortion(angle-25.0, night)
		diff = self.timeDiff(time, base) if direction == 'ccw' else self.timeDiff(base, time)
		if math.isnan(time) or diff > portion:
			time = base + (-portion if direction == 'ccw' else portion)
		return time
...
if __name__ == "__main__":
	...
	for i in ['Fajr', 'quran', 'Sunrise', 'Dhuhr', 'Asr', 'Maghrib', 'Isha', 'Midnight']:
